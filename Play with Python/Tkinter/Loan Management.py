...
class Customer:
    def __init__(self,root):
        ...
        self.root.geometry("1250x720+0+0")
        title=Label(self.root,text="Loan Management System",font=("times new romman",20,'bold'),bd=10,relief=GROOVE,bg='yellow',fg='red')
...
#=====================Details of Customer=====================
        ...
        Detail_F.place(x=10, y=60, width=420, height=580)

        lbl_id = Label(Detail_F, text="Loan Id",font=("times new romman", 13, 'bold'))
        ...
        lbl_name = Label(Detail_F, text="Full Name",font=("times new romman", 13, 'bold'))
        ...
        lbl_mob = Label(Detail_F, text="Mobile No.",font=("times new romman", 13, 'bold'))
        ...
        lbl_aa = Label(Detail_F, text="Aadhar No.",font=("times new romman", 13, 'bold'))
        ...
        lbl_add = Label(Detail_F, text="Address", font=("times new romman", 13, 'bold'))
        ...
        lbl_pin = Label(Detail_F, text="PinCode", font=("times new romman", 13, 'bold'))
        ...
        lbl_amount = Label(Detail_F, text="Amount of Loan", font=("times new romman", 13, 'bold'))
        ...
        lbl_time = Label(Detail_F, text="Number of years", font=("times new romman", 13, 'bold'))
        ...
        lbl_rate = Label(Detail_F, text="Interest Rate", font=("times new romman", 13, 'bold'))
        ...
        lbl_Mp = Label(Detail_F, text="Monthly Payment", font=("times new romman", 13, 'bold'))
        ...
        lbl_tp = Label(Detail_F, text="Total Payment", font=("times new romman", 13, 'bold'))
...
#=========================Employee Record======================
        ...
        recordFrame.place(x=435, y=60, width=810, height=430)
        ...
        btnFrame.place(x=435, y=500, width=810, height=80)
...
#=============================Functions================================
    ...
    def addrecord(self):
        if self.loalId.get() == '' or self.name.get() == '' or self.mob.get() == '' or self.aadhar.get() == '' or self.add.get() == '' or self.pin.get()=='':
            ...
            self.total()
            con = mysql.connector.connect(host="localhost", user="root", password="Lokesh@8788", database="dmp")
            cur = con.cursor()
    ...
    def update(self):
        if self.loalId.get()=='':
            ...
            cur.execute("update employee set name=%s,years=%s,rate=%s,mpay=%s,tpay=%s,mob= %s ,aadhar=%s,add=%s,pin=%s,amount=%s where loanId=%s",(self.name.get(),
                                                                                                                                                                         self.year.get(),
                                                                                                                                                                        self.rate.get(),
                                                                                                                                                                        self.mpay.get(),
                                                                                                                                                                        self.tpay.get(),
    ...
    def delete(self):
        if self.loalId.get()=='':
            ...
            con = mysql.connector.connect(host="localhost", user="root", password="Lokesh@8788", database="dmp")
            cur = con.cursor()
            cur.execute("delete from employee where loanId=%s",self.loalId.get())
            con.commit()
            con.close()
            self.fatch_data()
            self.reset()
